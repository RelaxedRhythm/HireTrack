...
export default function CandidatesTable({
  ...
  useEffect(() => {
    ...
    async function fetchDetail() {
      try {
        ...
        const res = await fetch(`/api/candidates/${selectedId}`);
        if (res.ok) {
  ...
  if (candidates.length === 0) {
    return <EmptyState title="No Candidates Found" description="Add candidate profiles to build your directory." />;
  }

  return (
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-210px)] min-h-[480px]">
            ...
            return (
              <button
                ...
                className={`w-full text-left p-4 transition-all flex items-start gap-3.5 hover:bg-muted/30 group outline-none ${
                  isSelected ? "bg-muted/70 border-l-2 border-primary" : "border-l-2 border-transparent"
                }`}
              >
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[10px] bg-secondary/50 border border-border/30 px-1.5 py-0.5 rounded-full text-muted-foreground font-semibold tracking-wide shrink-0">
                      {candidate._count?.applications ?? 0} {candidate._count?.applications === 1 ? 'app' : 'apps'}
                    </span>
                  </div>
      ...
      <div className="flex-1 border border-border/40 rounded-xl bg-card flex flex-col overflow-hidden">
        {detailLoading ? (
          <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground">
            ...
            <span className="text-xs mt-2 font-medium">Fetching candidate details...</span>
          </div>
        ) : selectedDetail ? (
            ...
            <div className="border-b border-border/30 bg-muted/10 p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="flex h-14 w-14 items-center justify-center rounded-full bg-secondary text-base font-bold text-secondary-foreground border border-border/40">
                  {selectedDetail.name.split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 2)}
                </div>
                
                <div>
                  <h3 className="text-lg font-bold tracking-tight text-foreground">{selectedDetail.name}</h3>
              ...
              <Button size="sm" variant="outline" asChild className="shrink-0">
                <Link href={`/candidates/${selectedDetail.id}`} className="inline-flex items-center gap-1.5">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              {(selectedDetail.resumeUrl || selectedDetail.linkedinUrl) && (
                <div className="space-y-2">
                  <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Documents & Links</h4>
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Job Applications</h4>
                {selectedDetail.applications && selectedDetail.applications.length > 0 ? (
                  <div className="space-y-3">
                    {selectedDetail.applications.map((app: any) => (
                      <div 
                        key={app.id}
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-semibold tracking-tight text-foreground">{app.job.title}</span>
                          </div>
                        ...
                        {app.rating !== null && (
                          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                            ...
                            <div className="flex items-center gap-0.5 text-amber-500">
                              ...
                              <span className="font-semibold text-foreground">{app.rating}/10</span>
                            </div>
                          </div>
                        )}
                        ...
                        {app.notes && (
                          <div className="text-xs bg-muted/40 border border-border/30 rounded p-2.5 mt-1">
                            <span className="font-semibold text-muted-foreground block mb-0.5">Recruiter Notes:</span>
                            <p className="text-muted-foreground italic leading-relaxed">{app.notes}</p>
                          </div>
                        )}
